(function () {
    ...
    if (prevLink) prevLink.href = 'wedding-schedule.html?month=' + (prevMonth.getMonth() + 1) + '&year=' + prevMonth.getFullYear();
    if (nextLink) nextLink.href = 'wedding-schedule.html?month=' + (nextMonth.getMonth() + 1) + '&year=' + nextMonth.getFullYear();

    // Preview only: sample Thursday weddings, not live parish availability.
    const sampleEvents = [];
    for (let day = 1; day <= 31; day += 1) {
        const date = new Date(current.getFullYear(), current.getMonth(), day);
        if (date.getMonth() === current.getMonth() && date.getDay() === 4) {
            sampleEvents.push({ day, category: 'mass', title: 'Ordinary Wedding', time: 'Time to be confirmed', location: 'Parish Church' });
        }
    }
    ...
    if (listView) {
        ...
        if (monthEvents.length === 0) {
            ...
            empty.textContent = 'No scheduled weddings for this month yet.';
    ...
    const upcoming = document.getElementById('weddingUpcoming');
    ...
    viewButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (view === 'today') {
                window.location.href = 'wedding-schedule.html';
                return;
            }
