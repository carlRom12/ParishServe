(function () {
    ...
    if (prevLink) prevLink.href = 'confirmation-schedule.html?month=' + (prevMonth.getMonth() + 1) + '&year=' + prevMonth.getFullYear();
    if (nextLink) nextLink.href = 'confirmation-schedule.html?month=' + (nextMonth.getMonth() + 1) + '&year=' + nextMonth.getFullYear();

    // Sample Confirmation-related activities shown on the 6th/13th/20th/27th
    // of whichever month is displayed -- the portal has no live schedule
    // data source yet, so this is illustrative content, not a real feed.
    const sampleEvents = [
        { day: 6, category: 'orientation', title: 'Confirmation Orientation', time: '9:00 - 11:00 AM', location: 'Parish Hall' },
        { day: 13, category: 'recollection', title: 'Recollection for Candidates', time: '8:00 AM - 12:00 PM', location: 'Parish Church' },
        { day: 20, category: 'mass', title: 'Confirmation Mass', time: '10:00 AM - 12:00 NN', location: 'Parish Church' },
        { day: 27, category: 'other', title: 'Thanksgiving Mass (Confirmands)', time: '9:00 - 11:00 AM', location: 'Parish Church' },
    ];
    ...
    if (listView) {
        ...
        if (monthEvents.length === 0) {
            ...
            empty.textContent = 'No scheduled Confirmation activities for this month yet.';
            listView.append(empty);
        } else {
            monthEvents.slice().sort((a, b) => a.day - b.day).forEach(event => {
                const item = document.createElement('div');
                item.className = 'conf-sched-event-item';
    ...
    viewButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (view === 'today') {
                window.location.href = 'confirmation-schedule.html';
                return;
            }
            viewButtons.forEach(other => {
                if (other.dataset.view !== 'today') other.classList.toggle('active', other === button);
            });
