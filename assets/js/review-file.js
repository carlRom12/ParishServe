(async function () {
    const funeral = location.pathname.includes('funeral-request');
    const key = (funeral ? 'funeral' : 'confirmation') + '-file-preview';
    const input = document.querySelector('input[type="file"]');
    const view = document.querySelector('.wr3-doc-view, .funeral-file-status');
    ...
    if (input) {
        ...
        form.addEventListener('submit', async event => {
            ...
            try {
                ...
                location.href = funeral ? 'funeral-request-step4.html' : 'confirmation-request-step4.html';
    ...
    if (!record || record.expires < Date.now()) {
        ...
        const link = document.createElement('a'); link.href = funeral ? 'funeral-request-step3.html' : 'confirmation-request-step3.html'; link.textContent = 'Select file'; view.replaceWith(link);
        return;
    }
